...
<body>
    ...
    if (isset($_REQUEST['username'])) {
        ...
        $query    = "INSERT INTO user (user_name, emailId, password, mobileNumber , OTP)"
            . "VALUES ('$username','$email','$passwordhash','$mobilenumber','$num');";
        $result = mysqli_query($con, $query);
        if ($result) {
            include 'sendotp.php';
            sendotp($email,$username,$num);
            session_start();
            $_SESSION["email"]=$email;
            echo "<div class='form'>
                  <h3>You are registered successfully and now please verify your Email</h3><br/>
                  <p class='link'>Click here to <a href='otp.php'> Verify </a></p>
                  </div>";
        } else {
            echo "<div class='form'>
    ...
    ?>
        ...
        <div ng-app="ngpatternApp" ng-controller="ngpatternCtrl">
            <form class="form" action="" method="post" name="personForm" novalidate ng-submit="personForm.$valid &&sendForm()" autocomplete="off">
                <a href="index.php">
                ...
                <input type="text" class="login-input" name="username" ng-model="username" ng-pattern="/^[a-zA-Z\s]{2,}$/" placeholder="Username" required /><br>
                ...
                <input type="password" class="login-input" name="password" ng-model="password" ng-pattern="/^(((?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])))(?=.{8,})/" placeholder="Password" required /><br>
